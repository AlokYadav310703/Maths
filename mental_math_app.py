...
if st.session_state.session_active and st.session_state.problems:
    ...
    if st.session_state.current_problem_idx < len(st.session_state.problems):
        ...
        if st.session_state.time_per_question:
            question_elapsed = time.time() - st.session_state.question_start_time
            remaining = st.session_state.time_per_question - question_elapsed
            
            if remaining <= 0:
                col3.markdown(f"<div class='timer-box' style='background-color: #D62828;'>Time's Up!</div>", unsafe_allow_html=True)
                ...
                timer_bg_color = "#D62828" if remaining < 5 else "#F18F01"
                timer_text = f"{remaining:.0f}s"
                col3.markdown(f"<div class='timer-box' style='background-color: {timer_bg_color};'>{timer_text}</div>", unsafe_allow_html=True)
                
                # Auto-rerun every 100ms to update timer display
                if not st.session_state.submitted_current:
                    time.sleep(0.1)
                    st.rerun()
        else:
            col3.markdown("<div class='timer-box' style='background-color: #4CAF50;'>No Timer</div>", unsafe_allow_html=True)
